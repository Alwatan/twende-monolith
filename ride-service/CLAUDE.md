...
- [ ] **1. application.yml** -- Configure port 8085, datasource `twende_rides`, Redis connection, Kafka bootstrap servers and consumer/producer settings (group-id `ride-service`), inter-service URLs (`pricing-service`, `location-service`, `loyalty-service`, `matching-service`, `country-config-service`)
- [ ] **2. Entities** -- `Ride` extending `BaseEntity` with ALL fields: riderId, driverId, status, vehicleType, cityId, pickup/dropoff lat/lng/address, estimatedFare, fareBoostAmount (default 0), finalFare, currencyCode, freeRide, freeRideOfferId, driverRejectionCount, tripStartOtpHash, tripStartOtpExpiresAt, tripStartOtpAttempts, distanceMetres, durationSeconds, requestedAt/assignedAt/arrivedAt/startedAt/completedAt/cancelledAt, cancelReason, cancelledBy, matchingTimeoutAt. `RideStatusEvent` with: rideId, fromStatus, toStatus, actorId, actorRole, metadata (JSONB), occurredAt. `RideDriverRejection` with: rideId, driverId, rejectedAt. Create Flyway migration `V1__create_ride_schema.sql` with all three tables and indexes
- [ ] **3. Repositories** -- `RideRepository`: findByIdAndRiderId, findByIdAndDriverId, findByStatusAndMatchingTimeoutAtBefore, findByRiderIdOrderByCreatedAtDesc (paginated), findByDriverIdAndStatusIn (current ride). `RideStatusEventRepository`: findByRideIdOrderByOccurredAtAsc. `RideDriverRejectionRepository`: existsByRideIdAndDriverId (idempotency check), findByRideId
- [ ] **4. Inter-service clients (RestClient)** -- `PricingClient`: GET `/internal/pricing/estimate` (fare estimate), POST `/internal/pricing/calculate` (final fare). `LocationClient`: GET `/internal/locations/geofence/check` (service area + RESTRICTED zone validation). `LoyaltyClient`: GET `/internal/loyalty/offers/applicable` (find free ride offer), POST `/internal/loyalty/offers/{id}/redeem` (redeem offer). `ConfigClient`: GET `/internal/config/vehicle-types/{country}/{type}` (max fare cap for boost validation). Configure all in `RestClientConfig` with base URLs from `twende.services.*`
- [ ] **5. RideService: createRide** -- Validate request fields. Call `LocationClient` for geofence check (reject if not in OPERATING zone or in RESTRICTED zone). Call `PricingClient` for fare estimate. Call `LoyaltyClient.findApplicableOffer()` -- if offer found, set freeRide=true, freeRideOfferId, estimatedFare=0, redeem offer atomically. Save ride with status=REQUESTED, set matchingTimeoutAt = now + 3min. Publish `RideRequestedEvent` to Kafka. Return rideId + estimated fare
- [ ] **6. RideService: status transitions** -- `assignDriver()` (REQUESTED -> DRIVER_ASSIGNED, set driverId, assignedAt). `markArrived()` (DRIVER_ASSIGNED -> DRIVER_ARRIVED, set arrivedAt, trigger OTP generation). `completeRide()` (IN_PROGRESS -> COMPLETED, calculate final fare, set distanceMetres, durationSeconds, completedAt, publish RideCompletedEvent). `cancelRide()` (REQUESTED/DRIVER_ASSIGNED/DRIVER_ARRIVED -> CANCELLED, apply cancellation fee rules, set cancelledAt/cancelReason/cancelledBy, publish RideCancelledEvent). `markNoDriverFound()` (REQUESTED -> NO_DRIVER_FOUND). Log every transition to `ride_status_events` table with actor info
- [ ] **7. FareBoostService** -- `boostFare()`: validate ride is in REQUESTED status, validate boostAmount > 0 (upward only), fetch `VehicleTypeConfig` from `ConfigClient` for max fare cap (baseFare * surgeMultiplierCap), validate newFare does not exceed cap, add boostAmount to cumulative fareBoostAmount, update estimatedFare, save ride, publish `RideFareBoostedEvent` to Kafka topic `twende.rides.fare-boosted`
- [ ] **8. TripOtpService** -- `generateAndSendOtp()`: generate 4-digit code (SecureRandom), bcrypt hash (cost 4), set tripStartOtpHash, tripStartOtpExpiresAt = now + 10min, tripStartOtpAttempts = 0, save ride, publish notification event to Kafka with OTP in data payload. `verifyOtp()`: check DRIVER_ARRIVED status, check expiry (if expired: regenerate + send + throw), check hash match (if wrong: increment attempts, if >= 3 attempts: regenerate + send + throw, else throw with remaining count), on success: set IN_PROGRESS, startedAt, null out otpHash (single-use), save. `resendOtp()`: regenerate and send new OTP to rider
- [ ] **9. Rejection handling** -- Kafka consumer for `twende.drivers.rejected-ride`: idempotency check via `RideDriverRejectionRepository.existsByRideIdAndDriverId()`, increment `driverRejectionCount` on ride, save `RideDriverRejection` record, publish rejection count update notification to rider. At count == 3: publish fare boost nudge notification to rider
- [ ] **10. Ride completion** -- Call `PricingClient.calculateFinalFare()` with actual distanceMetres and durationSeconds. Set finalFare, completedAt. Publish `RideCompletedEvent` to Kafka (includes freeRide flag and freeRideOfferId for payment-service to handle wallet credit)
- [ ] **11. Kafka consumers** -- `RideOfferAcceptedConsumer` (topic `twende.rides.offer-accepted`): delegate to `assignDriver()` with idempotency (no-op if already DRIVER_ASSIGNED to same driver). `DriverRejectedRideConsumer` (topic `twende.drivers.rejected-ride`): delegate to rejection handling logic
- [ ] **12. Kafka producers** -- Publish to: `twende.rides.requested` (RideRequestedEvent on ride creation), `twende.rides.status-updated` (RideStatusUpdatedEvent on any transition), `twende.rides.completed` (RideCompletedEvent), `twende.rides.cancelled` (RideCancelledEvent), `twende.rides.fare-boosted` (RideFareBoostedEvent). Use `KafkaTemplate<String, Object>` with rideId as key
- [ ] **13. RideController** -- Rider endpoints: `POST /api/v1/rides` (create), `GET /api/v1/rides/{id}` (status with rejectionCount, boostAmount), `PUT /api/v1/rides/{id}/boost` (fare boost), `DELETE /api/v1/rides/{id}` (cancel), `GET /api/v1/rides/history` (paginated), `POST /api/v1/rides/{id}/otp/resend`. Driver endpoints: `GET /api/v1/rides/current`, `POST /api/v1/rides/{id}/accept` (delegates to matching-service), `POST /api/v1/rides/{id}/reject` (delegates to matching-service), `PUT /api/v1/rides/{id}/arrived`, `POST /api/v1/rides/{id}/start` (OTP), `PUT /api/v1/rides/{id}/complete`. All return `ApiResponse<T>`
- [ ] **14. Internal endpoints** -- `PUT /internal/rides/{id}/offer-accepted` (called by matching-engine, idempotent driver assignment). `PUT /internal/rides/{id}/driver-rejected` (increment rejection counter, idempotent). `GET /internal/rides/{id}` (full ride detail for payment, compliance)
- [ ] **15. DTOs + MapStruct mapper** -- Request DTOs: `CreateRideRequest`, `BoostFareRequest`, `StartTripRequest` (with `@Valid` annotations). Response DTOs: `RideResponse`, `RideStatusResponse`, `FareBoostResponse`, `RideHistoryResponse`. `RideMapper` interface with MapStruct for entity-to-DTO conversion
- [ ] **16. Unit tests + integration tests** -- Unit tests: state machine transitions (every valid + invalid transition), `boostFare()` (wrong status, negative amount, exceeds cap, not ride owner, multiple boosts), `startTripWithOtp()` (correct OTP, wrong OTP, expired OTP, max attempts then regeneration, single-use nulling), rejection counter increment and nudge at count == 3, free ride offer detection and redemption, cancellation fee logic. Integration tests with Testcontainers (Postgres + Redis + Kafka): full ride lifecycle (create -> assign -> arrive -> OTP -> complete), fare boost -> re-broadcast -> accept, 3 rejections -> nudge published, free ride creation and completion, matching timeout -> NO_DRIVER_FOUND, cancellation scenarios
- [ ] **17. Dockerfile** — Multi-stage build (eclipse-temurin:21-jdk-alpine for build, 21-jre-alpine for run). Non-root `twende` user. Health check on `/actuator/health`. Expose port 8085.
- [ ] **18. OpenAPI config** — `OpenApiConfig.java` with SpringDoc `OpenAPI` bean. Title: "Ride Service API". Swagger UI at `/swagger-ui.html`.
- [ ] **19. Verify** -- Run `./mvnw -pl ride-service clean verify`, confirm all tests pass and JaCoCo coverage meets 80% threshold
